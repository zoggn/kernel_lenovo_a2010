...
#if defined(BUILD_LK)
			  printf("ili9806e_yaxin_dsi lk -- ili9806e 0x%x , 0x%x , 0x%x, 0x%x \n", id_high, id_midd, id_low, id);
#else
			  printk("ili9806e_yaxin_dsi kernel -- ili9806e 0x%x , 0x%x , 0x%x, 0x%x \n", id_high, id_midd, id_low, id);
#endif

        if((LCM_ID_ILI9806E == id) && (lcm_adc < 100))
		return 1;
	else
		return 0;
}
...
// ---------------------------------------------------------------------------
LCM_DRIVER ili9806e_yaxin_hd720_dsi = {
